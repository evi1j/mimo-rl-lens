...
**对话会落库**（`coach_msg` 表），所以刷新页面能接着聊：前端的 `msgs` 只在内存里，
页面加载时去 `GET /api/coach/history` 取回最近 60 条渲染，并插一条「以上是上次的对话」
分隔——新旧一眼能分开。只存纯文本 `{ role, content }`：思考过程与工具调用每轮都会
重新生成，存下来既占地方，又会把上一轮查到的旧数字带回下一轮（数字应当现查）。
只写思考、正文空白的那轮不落库，否则刷新回来会看到一条空回答。库里只留最近 200 条。

抽屉顶部「清空」是**两态确认**：点一次变成「再点一次」，3 秒不理自己复原 ——
比 `confirm()` 好的地方是它不打断操作，而且 jsdom 里能测（`confirm` 根本不存在）。
清空会同时清掉库里那份，否则刷新一下旧对话又回来了。
