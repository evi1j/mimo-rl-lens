...
**回答的排版**：教练正文在前端做一次轻量渲染（`public/coach.js` 的 `rich()`），只认四样 ——
`### 小标题`、行首「- 」列表、`**加粗**`、markdown 表格。
这四样是**前后端约定**，要动必须两边一起动：只改前端模型不会输出，只改提示词语法会原样露出。
表格识别要求两道条件齐全（该行以竖线开头 **且** 下一行是 `|---|` 分隔行），
免得正文里偶发的竖线被误判；列数以表头为准，缺列补空、多列截掉。

**渲染的已知边界**（想扩之前先看这里）：有序列表 `1.`、嵌套列表缩进、行内链接、斜体、
代码块、LaTeX 都不渲染，会原样显示。另外渲染只发生在收尾那一次 ——
流式期间上屏的是纯文本，所以表格在生成途中先以一堆竖线出现，收尾才成形。
