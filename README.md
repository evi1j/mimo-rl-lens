...
| 水位 | `used = 系统提示词 + 摘要 + 历史 + 本轮问题 + 预留`（预留 = 本轮生成 + 工具返回，最多占窗口一半）。`/api/coach` 开头会推一条 `{"context":{...}}`，前端把它画成一条细进度条 + 百分比（不做成按钮的样子：它是读数不是操作），60% 起变黄、80% 起变红，悬浮有详细说明 |
...
滚动条统一成跟主题走的一套）。
→ `v1.7.2`（教练抽屉改成挤压式：打开时页面给右边让位、内容重排，不再盖住图表；
左边缘可拖宽度，380px ~ 视口 82%，记在 localStorage）。
→ `v1.7.3`（教练顶部重排：切换会话从「下拉」改成「点开列表、一段一行」，
能看见名字/条数/时间、还能删；水位从一个像按钮的胶囊改成细进度条；
...
链接显示的文字一律由 href 推导）。
→ `v1.7.6`（上下文水位修好：原先百分比里算进了「每轮固定留给工具与回答的那一万
多 token」，所以空会话也显示 44%、聊长了也不怎么动；而且触发线按整个窗口的 75%
算，比真正能用的空间还大，等于永远压不到。现在水位线、百分比都按扣掉预留后的
可用空间算，条上还画出触发线的位置）。
→ `v1.7.7`（教练入口搬到顶栏后，页面滚动时 `nav.topnav` 的 sticky 吸顶盖住了顶栏，
导致 AI 模型训练教练按钮一下拉就消失。现在顶栏 z-index 提到 40、导航条吸在顶栏
底下 67px 处、侧边栏吸在两条导航之下，各层不再互抢）。
→ `v1.7.8`（指标讲解抽屉关掉后不再自己弹回来：页面每 10s 拉一次数据，拉完会重画
抽屉让里面数值跟上，而重画顺手就把它显示出来了 —— 关掉时 `glCur` 是故意留着的，
所以必须另有一个「抽屉开着吗」的标志挡住这类重画）。
→ `v1.7.9`（打开 AI 教练后再点指标图表讲解「没显示」：教练抽屉也是右侧 fixed，
而且 z-index（71）比讲解高，讲解会被它整块压在底下。现在教练开着时讲解盖在教练
之上，并至少和教练一样宽；关掉讲解教练原样还在）。
